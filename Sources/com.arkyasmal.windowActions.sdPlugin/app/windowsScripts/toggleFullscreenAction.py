...
filePath = os.path.join(dataDirectory, r"Elgato\StreamDeck\Plugins\com.arkyasmal.windowactions.sdPlugin\currFullScreenWindows.json")
...
def fullscreen_off(hwnd: str, currFullScreenWindows: dict):
    #Set the window styles
    windowStyles = currFullScreenWindows[hwnd]["windowStyles"]
    extendedWindowStyles = currFullScreenWindows[hwnd]["extendedWindowStyles"]
...
def toggle_fullscreen_windows(win_id_type: str, win_id: str):
    ...
    #activate fullscreen
    result = [toggle_fullscreen(i['hWnd'], currFullScreenWindows) for i in matching_windows]
    #save data to file
