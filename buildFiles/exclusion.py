...
def merge(a: dict, b: dict, path=[]):
    for key in b:
        if key in a:
            ...
            a[key] = b[key]
    return a
def merge_all(dictionaries: [dict]):
    result = {}
...
def relative_to_top(path: str):
    ...
    new_path = os.path.normpath(os.path.abspath(os.path.join(curr_dir,fr'..\{path}')))
    return get_directory_tree(new_path)
dir_to_ignore = merge_all([
    relative_to_top(r'Sources\com.arkyasmal.windowactions.sdPlugin\app\buildFiles'),
    relative_to_top(r'Sources\com.arkyasmal.windowactions.sdPlugin\app\dll'),
    relative_to_top(r'Sources\com.arkyasmal.windowactions.sdPlugin\app\windowsScripts'),
